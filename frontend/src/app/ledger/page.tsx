...
export default function ActionLedgerPage() {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        setFetchError(null);
        const data = await fetchApi<LedgerRecord[]>('/api/v1/ledger');
        setLogs(data);
      } catch (e) {
        console.error('Failed to fetch action ledger:', e);
        setFetchError('Unable to load action ledger');
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="space-y-6 max-w-5xl mx-auto font-mono">
      ...
      <div className="bg-card border border-subtle rounded-xl p-6 space-y-6 shadow-sm">
        ...
        <div className="relative pl-6 space-y-6 border-l-2 border-subtle">
          {loading ? (
            <div className="p-4 text-center text-muted font-sans italic">
              Loading action ledger...
            </div>
          ) : fetchError ? (
            <div className="p-4 text-center text-rose-500 font-sans italic">
              {fetchError}
            </div>
          ) : logs.length === 0 ? (
            <div className="p-4 text-center text-muted font-sans italic">
              No actions recorded yet. Actions will appear here when containment strategies are executed.
            </div>
          ) : (
            logs.map((log) => (
