...
import { NetworkScene, networkLabels } from '@/components/landing/NetworkScene';
  ...
  return (
    <main className="min-h-screen bg-app text-primary">
      <section className="mx-auto grid min-h-[calc(90vh-88px)] max-w-7xl items-center gap-10 px-6 py-12 lg:grid-cols-[.88fr_1.12fr] lg:px-12 lg:py-16">
        <div className="animate-draw-in"><p className="mb-5 font-mono text-xs tracking-[0.18em] text-accent-blue">REAL-TIME PAYMENT RISK INTELLIGENCE</p><h1 className="max-w-2xl font-display text-6xl leading-[.98] tracking-tight text-balance md:text-8xl">Fraud rarely looks suspicious <em className="text-muted">one payment at a time.</em></h1><p className="mt-7 max-w-xl text-base leading-7 text-muted md:text-lg">RISKGRAPH connects payments, accounts, devices, IPs and payment infrastructure to uncover coordinated attacks that isolated transaction scoring misses.</p><div className="mt-8 flex flex-wrap items-center gap-5"><Link href="/dashboard" className="bg-accent-blue px-5 py-3 text-xs font-bold text-surface-primary no-underline hover:opacity-90">Open Risk Console <ArrowUpRight className="ml-2 inline size-3.5" /></Link><a href="#demo" className="border-b border-primary pb-1 text-xs font-semibold no-underline">Watch the attack simulation</a></div></div>
        <div className="relative min-h-[500px] overflow-hidden border border-subtle bg-surface lg:min-h-[580px]"><NetworkScene /><div className="pointer-events-none absolute inset-0 bg-grid-pattern opacity-20" />{networkLabels.map(({ label, x, y }) => <div key={label} className="pointer-events-none absolute -translate-x-1/2 -translate-y-1/2 bg-surface/90 px-2 py-1 font-mono text-[9px] tracking-[0.16em] text-muted" style={{ left: x, top: y }}>{label}</div>)}<div className="absolute bottom-5 left-6 right-6 flex justify-between border-t border-subtle pt-3 font-mono text-[9px] text-muted"><span>RELATIONSHIP GRAPH / LIVE</span><span>PAYMENT NETWORK / 5 ENTITIES</span></div></div>
      </section>
