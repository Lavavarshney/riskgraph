...
import { ArrowRight, ArrowUpRight, Check, CircleDot, Play, ShieldCheck, Square, Waypoints } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-app text-primary">
      <header className="mx-auto flex max-w-7xl items-center justify-between border-b border-subtle px-6 py-6 lg:px-12">
        <Link href="/" className="flex items-center gap-3 no-underline"><ShieldCheck className="size-5 text-accent-blue" /><span className="font-bold tracking-[0.18em] text-sm">RISKGRAPH</span></Link>
        <nav className="hidden items-center gap-8 text-xs text-muted md:flex"><a href="#problem" className="hover:text-primary">Why it matters</a><a href="#method" className="hover:text-primary">How it works</a><Link href="/dashboard" className="flex items-center gap-1 font-semibold text-primary no-underline">Open console <ArrowUpRight className="size-3.5" /></Link></nav>
      </header>

      <section className="mx-auto grid max-w-7xl items-center gap-14 px-6 py-20 lg:grid-cols-[.9fr_1.1fr] lg:px-12 lg:py-28">
        <div className="animate-draw-in"><p className="mb-6 font-mono text-xs tracking-[0.18em] text-accent-blue">REAL-TIME PAYMENT RISK INTELLIGENCE</p><h1 className="max-w-2xl font-display text-5xl leading-[1.02] tracking-tight text-balance md:text-7xl">Fraud rarely looks suspicious <em className="text-muted">one payment at a time.</em></h1><p className="mt-8 max-w-xl text-base leading-7 text-muted md:text-lg">RISKGRAPH connects payments, accounts, devices, IPs and payment infrastructure to uncover coordinated attacks that isolated transaction scoring misses.</p><div className="mt-10 flex flex-wrap items-center gap-5"><Link href="/dashboard" className="bg-accent-blue px-5 py-3 text-xs font-bold text-surface-primary no-underline hover:opacity-90">Open Risk Console <ArrowUpRight className="ml-2 inline size-3.5" /></Link><a href="#demo" className="border-b border-primary pb-1 text-xs font-semibold no-underline">Watch the attack simulation</a></div></div>
        <div className="relative min-h-[390px] overflow-hidden border border-subtle bg-surface"><NetworkScene /><div className="pointer-events-none absolute inset-0 bg-grid-pattern opacity-20" /><div className="absolute left-[17%] right-[17%] top-[34%] border-t border-accent-blue/50 rotate-[20deg]" /><div className="absolute left-[17%] right-[17%] top-[64%] border-t border-danger/40 rotate-[-18deg]" /><div className="absolute left-[22%] right-[22%] top-1/2 border-t border-warning/50" />{[['ACCOUNT','18%','23%'],['PAYMENT','51%','43%'],['DEVICE','78%','23%'],['IP','18%','73%'],['INFRASTRUCTURE','78%','73%']].map(([label, x, y], index) => <div key={label} className="absolute flex flex-col items-center gap-2" style={{ left: x, top: y }}><div className={`size-3 rounded-full ${entities[index]?.tone ?? 'bg-accent-blue'} ring-8 ring-surface`} /><span className="whitespace-nowrap font-mono text-[9px] tracking-wider text-muted">{label}</span></div>)}<div className="absolute bottom-5 left-6 right-6 flex justify-between border-t border-subtle pt-3 font-mono text-[9px] text-muted"><span>RELATIONSHIP GRAPH / LIVE</span><span>5 connected entities</span></div></div>
      </section>
