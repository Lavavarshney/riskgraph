  ...
  return (
    <main className="min-h-screen bg-app text-primary">
      <section className="mx-auto grid min-h-[calc(90vh-88px)] max-w-7xl items-center gap-10 px-6 py-12 lg:grid-cols-[.88fr_1.12fr] lg:px-12 lg:py-16">
        ...
        <div className="relative min-h-[500px] overflow-hidden border border-subtle bg-surface lg:min-h-[580px]"><NetworkScene phase={phase} /><div className="pointer-events-none absolute inset-0 bg-grid-pattern opacity-20" />{networkLabels.map(({ label, x, y }, index) => <div key={`${label}-${index}`} className="pointer-events-none absolute -translate-x-1/2 -translate-y-1/2 bg-surface/90 px-2 py-1 font-mono text-[9px] tracking-[0.16em] text-muted" style={{ left: x, top: y }}>{label}</div>)}<div className="absolute bottom-5 left-6 right-6 flex justify-between border-t border-subtle pt-3 font-mono text-[9px] text-muted"><span>RELATIONSHIP GRAPH / LIVE</span><span>PAYMENT NETWORK / 5 ENTITIES</span></div></div>
      </section>
